...
import LatestCar from "../../Component/LatestCar/LatestCar";
import { Link } from "react-router";

const latestCars = fetch("http://localhost:3000/latest-car").then((res) =>
  res.json()
);
const Home = () => {
  return (
        ...
        <div className="relative z-10 flex flex-col justify-center h-full px-6 md:px-16 text-white mx-auto container py-6">
          ...
          <Link to={"/all-cars"}>
            <motion.button
              ...
              className="mt-8 w-fit flex items-center gap-2 px-6 py-3 bg-white text-black rounded-full font-semibold hover:bg-gray-100 transition cursor-pointer"
            >
              View All Cars <span className="text-xl">→</span>
            </motion.button>
          </Link>
        </div>
      </div>
      {/* Latest Products */}
      <LatestCar latestCars={latestCars} />
    </>
  );
};
